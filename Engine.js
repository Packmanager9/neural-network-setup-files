...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Network {
        constructor(inputs, layerSetupArray){
            ...
            for(let t = 0;t < layerSetupArray.length; t++){
                ...
                for(let k = 0; k< layerSetupArray[t]; k++){
                    let perceptron
                    if (t == 0){
                         perceptron = new Perceptron(this.inputs)
                    }else{
                        perceptron = new Perceptron(this.structure[t-1])
                    }
                    this.scaffold.push(perceptron)
                }
        ...
        clone(net){
            let newNet = new Network([0,0,0,0], [4,4,2]) ///the same setup, and input size, as the origin
            for(let t = 0;t<net.structure.length; t++){
                for(let k = 0;k<net.structure[t].length; k++){
                    newNet.structure[t][k] = new Perceptron(net.inputs)
                    for(let n = 0; n <net.structure[t][k].weights.length;n++){
                        newNet.structure[t][k].weights[n] = net.structure[t][k].weights[n]
                        newNet.structure[t][k].bias = net.structure[t][k].bias
                    }
                }
            }
            return newNet
            // this.network = target.network.clone(target.network)
        }
        mutate(){
            ...
            this.structure[perceptronLayer][perceptronNumber].weights[perceptronWeightNumber] = (this.structure[perceptronLayer][perceptronNumber].weights[perceptronWeightNumber]+this.structure[perceptronLayer][perceptronNumber].weight())/2
        }
    }
    class Data {
